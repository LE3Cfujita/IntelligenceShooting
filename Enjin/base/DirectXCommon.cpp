...
void DirectXCommon::Initislize(WinApp* winApp)
{
    //NULL検出
    assert(winApp);
    //メンバ変数に記録
...
void DirectXCommon::InitializeDevice()
{
    ...
    //DXGIファクトリーの生成 *補助的な仕組み
    result = CreateDXGIFactory1(IID_PPV_ARGS(&dxgiFactory));

    //アダプターの列挙用　*アダプターをすべて見つけたものを入れておく配列みたいなもの
    std::vector<ComPtr<IDXGIAdapter1>> adapters;

    //ここに特定の名前を持つアダプターオブジェクトが入る　*順番に列挙して見つけたものを追加する
...
void DirectXCommon::InitializeSwapchain()
{
    ...
    result = dxgiFactory->CreateSwapChainForHwnd(
        ...
        nullptr,
        &swapchain1);
    //生成したIDXGISwapChain1のオブジェクトをIDGISwapChain4に変換する
...
void DirectXCommon::InitializeRenderTargetView()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        dev->CreateRenderTargetView(
            backBuffers[i].Get(),
            nullptr,
            CD3DX12_CPU_DESCRIPTOR_HANDLE(rtvHeaps->GetCPUDescriptorHandleForHeapStart(), i, dev->GetDescriptorHandleIncrementSize(heapDesc.Type)));
    }
}
...
void DirectXCommon::PreDraw()
{
    ...
    float clearColor[] = { 0.1f,0.25f,0.5f,0.0f };  //画面の色
